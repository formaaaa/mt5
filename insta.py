...
# Create DWX_ZeroMQ_Connector object for communication with MT4
zmq = DWX_ZeroMQ_Connector()


# NOTE: For this example, I'm assuming you've written the get_usd_rate function
#       similarly to your MT5 example. You might need to adapt it based on the exact
#       data you retrieve from MT4.
def get_usd_rate(quote_currency):
    if quote_currency == 'USD':
        return 1
    else:
        usd_tick = zmq._DWX_MTX_GET_BID_ASK_(_symbol=f"{quote_currency}USD")
        if usd_tick:
            return usd_tick['ask']
    return None


symbols = [...]  # Your symbols list

tick_data = {}

for symbol in symbols:
    tick = zmq._DWX_MTX_GET_BID_ASK_(_symbol=symbol)
    # NOTE: You might need to modify the code below to extract symbol info from MT4.
    #       MT4 and MT5 have different symbol properties.
    info = zmq._DWX_MTX_GET_SYMBOL_INFORMATION_(symbol)
...
headers = ["Symbol", "Type", "Price", "Spread", "Spread in Points", "Spread in USD", "Contract Size", "Min Volume",
...
for symbol, data in tick_data.items():
    ...
    ws.cell(row=row_num, column=3).value = data["price"]
    ws.cell(row=row_num, column=4).value = data["spread"]
    ws.cell(row=row_num, column=5).value = data["spread_in_points"]
    ws.cell(row=row_num, column=6).value = data["spread_in_usd_per_contract"]
    ws.cell(row=row_num, column=7).value = data["contract_size"]
    ws.cell(row=row_num, column=8).value = data["min_volume"]
    ws.cell(row=row_num, column=9).value = data["max_volume"]
    ws.cell(row=row_num, column=10).value = data["limit_volume"]
    ws.cell(row=row_num, column=11).value = data["margin"]
    ws.cell(row=row_num, column=12).value = data["trade_calc_mode"]
    ws.cell(row=row_num, column=13).value = data["quote_currency"]
    ws.cell(row=row_num, column=14).value = data["usd_rate"]
    ws.cell(row=row_num, column=15).value = data["commission"]
    ws.cell(row=row_num, column=16).value = data["swap_long"]
    ws.cell(row=row_num, column=17).value = data["swap_short"]
    ws.cell(row=row_num, column=18).value = data["underlying_lot_amount_usd"]
    ws.cell(row=row_num, column=19).value = data["ib_commission_per_lot"]

    # Apply the yellow fill to the appropriate cells
    ws.cell(row=row_num, column=2).fill = yellow_fill  # Type column
    ws.cell(row=row_num, column=15).fill = yellow_fill  # Commission column
    ws.cell(row=row_num, column=19).fill = yellow_fill  # IB Commission per lot column
